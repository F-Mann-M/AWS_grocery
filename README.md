...
* **Secure Secrets Management:** Migrate sensitive environment variables (database credentials, JWT keys) out of the GitHub Actions pipeline. Store them securely in **AWS Secrets Manager**, allowing EC2 instances to fetch them dynamically at boot using IAM role permissions.
